      PROGRAM krassis
* for Ag in water
* revised version by A. Moroz 28 04 2001
C--------/---------/---------/---------/---------/---------/---------/--
      IMPLICIT NONE  
C     ------------------------------------------------------------------  
C     A B S T R A C T   
...
* kv
c	write(6,*)
c      write(6,*)'Filling fraction <= 0.74'
c      read (5,*) ff
      ff=0.6d0
c      if (nstack.eq.0) 
c     & rmuf=(3.d0*ff/(4.d0*pi))**(1.d0/3.d0)*sqrt(2.d0)       !sc lattice 
c     rmuf=(3.d0*ff/pi)**(1.d0/3)/sqrt(2.d0)                   !bcc lattice
      if (nstack.eq.1.or.nstack.eq.2)
     &   rmuf=(3.d0*ff/(2.d0*pi))**(1.d0/3)/sqrt(2.d0)        !fcc lattice 
	write(6,*) rmuf
***************************
...
**KV************** Input values *******************************
c	write(6,*)
c      write(6,*)'READ CORE-TO-SHELL RATIO'
c      read (5,*) rff
c	write(6,*)
c	write(6,*)
c	if (NMAT.EQ.4) then
c		write(6,*)'READ FILLING FRACTION OF ZnS IN THE ZnS-SILICA CORE'
c		read (5,*) FILFRAC
c		filfrac = 0.62d0
c	else if  (NMAT.EQ.2) then
c		write(6,*)'READ FILLING FRACTION OF Ag IN THE PARTICLE'
c		read (5,*) ff
c	ff = 0.8d0
c	end if
*
...
* Microslides 0.2 mm path length Width	d = 0.2 mm
* Large microslides Width					d = 0.8 - 1.0 mm
      D(1)=0.2d0/(2.d0*rsnm)   !True only for close-packed lattices
c      D(1)=0.d0
      D(3)=0.D0
* reference vectors
       DL(1,icomp,1)=0.d0
...
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF=2.d0*pi*rsnm/(990.d0*rmuf)
      ZSUP=2.d0*pi*rsnm/(500.d0*rmuf)
...
c      if (lcs.gt.1) write(6,*)'Coated sphere'
      ...
      write(6,*)'Modify dims of JL, NL, UL, VL, etc to LMAXD=',LMAX
      stop
      end if 
...
C  
      ...
      PARAMETER (LMAXD=14,LMAX1D=LMAXD+1,LMODD=(LMAXD*LMAX1D)/2)  
      PARAMETER (LM1SQD=LMAX1D*LMAX1D,IGD=31,IGKD=2*IGD,NELMD=165152)  
