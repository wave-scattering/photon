...
C
C     Check 
C           rev,ynctsphr,Q0,DEFP,RMUF,REV,FCT,NUNIT,ICOMPL,ynrealeps
C     before running
...
C     ync.eq.'y'   lcs.eq.1
C
C    Currently, the default facet option ensures that the first
C    and the last plane of scatterers has the width of at least RMUF,
C    where RMUF is the sphere radius in the program unit. In future,
C    make RMUF the halfwidth of a scatterer in the stacking direction
C     ------------------------------------------------------------------  
C     A B S T R A C T   
...
C The control via YNFCT is performed via numerical values of the constant FCT in routines
C SCAT.F and BAND.F
*
...
*       13               pyrochlore with 3 or 1 particles per layer
*       14               pyrochlore with 1 particle per layer 
...
C   C ..  SCALAR VARIABLES ..   C  
      ...
      INTEGER      IG0,NUNIT,ICOMP,ICOMPL,KEMB,IU,IP,IPL,IPLP,ILAYER,ILCSN
...
cc      read(5,*) rev
*
c      rev=170         !288.d0               !feeded as REV to RSP* routines
* 
c      AXI=rev
*
...
***************************
      ...
      rmuf=0.24999999999999999999d0     !cp pyrochlore 
...
C--------/---------/---------/---------/---------/---------/---------/-- 
      if (nstack.eq.0)  then
          al(3)=1.d0
      else if ((nstack.eq.1).or.((nstack.ge.7).and.(nstack.le.9))) then
          al(1)=(1.d0+cos(pi/3.d0))/3.d0
          al(2)=sin(pi/3.d0)/3.d0
          al(3)=2.d0/sqrt(6.d0)
      else if ((nstack.eq.2).or.(nstack.eq.10))   then
          al(3)=sqrt(2.d0)
      end if        !nstack
...
*
*
      if (NSTACK.lt.0.or.NSTACK.gt.15) write(6,*)'Illegal value of 
     & NSTACK'
*
      if (ynsp.eq.'y') then
...
*
      if ((ynphase).and.(ynsp.eq.'n')) then
      write(6,*)'The reflection phase calculation should be effective
     & if and only if the specular option YNSP is on!'
       end if
*
C======================================================================= 
*  OUTPUT WRITING
     ...
     & of a material dielectric constant was used in the calculation!!!'
      ...
      OPEN(UNIT=NOUT,FILE='output.dat')
      rewind(NOUT)
     ...
     &   write(nout,*)'#ZVAL=2*PI*ALPHA/LAMBDA; UNITS [c/ALPHA]'
      ...
      if ((ktype.eq.3).and.(ynrealeps)) write(nout,*)'#Only the REAL part
     & of a material dielectric constant was used in the calculation!!!'
      write(nout,*)'#'
* 
...
*
     ...
     & as an fcc lattice'
      ...
      else if (NSTACK.eq.13) then
      write(6,*)'Pyrochlore structure'
      write(nout,*)'Pyrochlore structure'
      end if
      else
...
* or
c       write(nout,*)'#Sphere 1: Ag core with rff=0.75;
c     &  Sphere 2: eps=1.45d0**2, rff=1'
c       write(nout,*)'#SiO2@Au@SiO2 sphere: rf1=181, rf2=237, rf3=274'
*
      end if 
...
***********************************************************************      
      ...
      OPEN(UNIT=40,FILE='pxSiAuSi-5uc.dat')
      rewind(40)
...
*
      ...
      WRITE(nout+1,*)'#Unit slice width in program units=', AL(3) 
      write(nout+1,*)'#'
...
C--------/---------/---------/---------/---------/---------/---------/--
      ...
      END IF
*
*write(nout+1,*)'#Ag bulk data'
      if(ync.eq.'y') then
        if (nmat.gt.0) then
        write(nout+1,*)'#The sphere component with material=',ilcs
        if (ilcs.gt.1) write(nout+1,*)'#sphere core diel. const.=',cceps
        if (ilcs.eq.1) write(nout+1,*)'#shell diel. const.=',cseps
        else
        write(nout+1,*)'#sphere core diel. constant=',cceps
        write(nout+1,*)'#shell diel. constant=',cseps
        end if
      else
      if(nmat.eq.0) write(nout+1,*)'#sphere core diel. constant=',cceps
      end if
*
      if (ynbrug) write(6,*)'Bruggeman approximation performed'
...
C                   PROGRAM UNITS: LAMDA/ALPHA  ),  ACCORDING  TO  THE  
C                   VALUE OF KSCAN. C AND LAMDA REFER TO VACUUM  
C     In band structure calculations (BSC), A=2 and 
C                                 OMEGA=pi*A/LAMBDA, 
C            WHERE, FOR AN FCC LATTICE   
C                             A=sqrt(2.d0)*ALPHA, 
C            WHEREAS HERE ALPHA=1, and hence A=SQRT(2), and
C                           ZVAL=2*PI*ALPHA/LAMBDA
C                     (I.E., FREQUENCY IS IN THE UNITS [C/ALPHA])
C                   THUS ZVAL here is related to OMEGA in BSC by 
C                             ZVAL=OMEGA_{BSC}*sqrt(2.d0)
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF= 0.2d0*sqrt(2.d0)
      ZSUP= 3.0d0*sqrt(2.d0)
c
...
c
      NSTEP=200
C     NSTEP          : NUMBER OF EQUALLY SPACED POINTS BETWEEN INF, ZSUP   * 
...
*  of a generic axisym particles (type specified by the NP parameter)
*  and the remaining layer having regular spheres. 
*  Later it will be useful to change ynctsphr,
...
* ZVAL=2*PI*ALPHA/LAMBDA ===> lambda=lambda*(rsnm/A)/rmuf=lambda
      ...
      write(67,*) 'Lambda in vacuum=', lambda    !temporarily

      if ((nmat.eq.0).or.(ynperfcon)) go to 80     !dispersionless dielectric 
                                                   !or ideal metal
...
*
      IF ((NMAT.GE.3).or.((nmat.ge.5).and.(nmat.le.7))) then   !Material decision IF 
                                                               !Au,Cu,Al,Pt 
c >>> real data:
...
*
      IPLP=1
c      write(6,*)'iplp= ',iplp
      CALL PCCSLAB(YNC,LMAX,IGMAX,NBAS,RAP,EPS1(1),EPSSPH(1,1),MU1(1)  
...
*--------/---------/---------/---------/---------/---------/---------/--
      ...
      if(mod(ipl,2).eq.0) iplp=2
      if(mod(ipl,2).eq.1) iplp=1
c      write(6,*)'iplp= ',iplp
*
...
*
                     ENDIF                    !KTYPE switch
*  
  300 CONTINUE                !end of SCANNING OVER FREQUENCIES/WAVELENGTHS loop
*
      write(6,*)
...
*  the whole sphere
*
      rff(1)=121.d0/171.d0     !228.d0/286.d0     !156.d0/221.d0                           
c      rff(2)=171.d0/181.d0     !266.d0/286.d0     !181.d0/221.d0                           
...
c      write(6,*)'CSEPS= ',CSEPS
c      write(6,*)'CSPHEPS= ',CSPHEPS
c      write(6,*)'CMEDEPS= ',CMEDEPS
*
      rmuf=1.d0
      rff(lcs)=1.d0
