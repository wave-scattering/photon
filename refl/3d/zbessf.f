*  Before using this routine, select appropriate type
*  of your machine in D1MACH and I1MACH functions
* SARA computer: Additional optimization may be attained by recompiling 
* and specifying MAXMEM option with a value greater than 2048
*
*
*DECK D1MACH
...
C
C***ROUTINES CALLED  ZACAI,ZBKNU,ZSEXP,ZSSQRT,I1MACH,D1MACH
...
C     COMPLEX AI,CONE,CSQ,CY,S1,S2,TRM1,TRM2,Z,ZTA,Z3
     ...
     * ZEROR, ZI, ZR, ZTAI, ZTAR, Z3I, Z3R, D1MACH, ZSABS, ALAZ, BB
...
C-----------------------------------------------------------------------
   ...
   40 CONTINUE
      ...
      IF (KODE.EQ.1) RETURN
      CALL ZSSQRT(ZR, ZI, STR, STI)
      ZTAR = TTH*(ZR*STR-ZI*STI)
      ZTAI = TTH*(ZR*STI+ZI*STR)
      CALL ZSEXP(ZTAR, ZTAI, STR, STI)
      PTR = AIR*STR - AII*STI
      AII = AIR*STI + AII*STR
      AIR = PTR
      RETURN
...
C
C***ROUTINES CALLED  D1MACH,ZSABS,ZDIV,ZSEXP,ZMLT,ZSSQRT
...
C     COMPLEX AK1,CK,CONE,CS1,CS2,CZ,CZERO,DK,EZ,P1,RZ,S2,Y,Z
     ...
     * S2R, TOL, TZI, TZR, YI, YR, ZEROI, ZEROR, ZI, ZR, D1MACH, ZSABS
...
C-----------------------------------------------------------------------
      ...
      AK1R = RTPI*STR*RAZ
      AK1I = RTPI*STI*RAZ
      CALL ZSSQRT(AK1R, AK1I, AK1R, AK1I)
      CZR = ZR
      CZI = ZI
...
C
C         CAN BE USED. HOWEVER, FOR LARGE ORDERS CLOSE TO INTEGERS,
...
C     COMPLEX CI,CSGN,CY,Z,ZN
     ...
     * D1MACH, BB, FN, AZ, ZSABS, ASCLE, RTOL, ATOL, STI
...
C
C***ROUTINES CALLED  ZSABS,ZASYI,ZBUNI,ZMLRI,ZSERI,ZUOIK,ZWRSK
C***END PROLOGUE  ZBINU
      DOUBLE PRECISION ALIM, AZ, CWI, CWR, CYI, CYR, DFNU, ELIM, FNU,
     * FNUL, RL, TOL, ZEROI, ZEROR, ZI, ZR, ZSABS
...
C
C***ROUTINES CALLED  DGAMLN,I1MACH,D1MACH,ZKSCL,ZSHCH,ZUCHK,ZSABS,ZDIV,
C                    ZSEXP,ZSLOG,ZMLT,ZSSQRT
...
C
     ...
     * TOL, TTH, T1, T2, YI, YR, ZI, ZR, DGAMLN, D1MACH, ZSABS, ELM,
...
C-----------------------------------------------------------------------
      FC = 1.0D0
      CALL ZSLOG(RZR, RZI, SMUR, SMUI, IDUM)
      FMUR = SMUR*DNU
      FMUI = SMUI*DNU
...
C-----------------------------------------------------------------------
  261 CONTINUE
      ...
      DO 262 I=1,INU
        ...
        CALL ZSLOG(S2R,S2I,STR,STI,IDUM)
        P2R = -ZDR+STR
        P2I = -ZDI+STI
        P2M = DEXP(P2R)/TOL
        P1R = P2M*DCOS(P2I)
...
C
C***ROUTINES CALLED  ZUNI1,ZUNI2,ZSABS,D1MACH
...
C     COMPLEX CSCL,CSCR,CY,RZ,ST,S1,S2,Y,Z
     ...
     * S2I, S2R, TOL, YI, YR, ZI, ZR, ZSABS, ASCLE, BRY, C1R, C1I, C1M,
     * D1MACH
...
C
C***ROUTINES CALLED  ZUCHK,ZUNIK,ZUOIK,D1MACH,ZSABS
...
C    *S2,Y,Z,ZETA1,ZETA2
     ...
     * ZETA1R, ZETA2I, ZETA2R, ZI, ZR, CYR, CYI, D1MACH, ZSABS
...
C     Y(I)=CZERO FOR I=NLAST+1,N
C
C***ROUTINES CALLED  ZAIRY,ZUCHK,ZUNHJ,ZUOIK,D1MACH,ZSABS
...
C        ZETA1,ZETA2.
C
C***ROUTINES CALLED  ZDIV,ZSLOG,ZSSQRT,D1MACH
...
C-----------------------------------------------------------------------
   ...
   15 CONTINUE
      ...
      CALL ZSSQRT(SR, SI, SRR, SRI)
      STR = CONER + SRR
      STI = CONEI + SRI
      CALL ZDIV(STR, STI, TR, TI, ZNR, ZNI)
      CALL ZSLOG(ZNR, ZNI, STR, STI, IDUM)
      ZETA1R = FNU*STR
      ZETA1I = FNU*STI
      ZETA2R = FNU*SRR
      ZETA2I = FNU*SRI
      ...
      SRR = TR*RFN
      SRI = TI*RFN
      CALL ZSSQRT(SRR, SRI, CWRKR(16), CWRKI(16))
...
C             ANOTHER ROUTINE
C
C***ROUTINES CALLED  ZUCHK,ZUNHJ,ZUNIK,D1MACH,ZSABS,ZSLOG
...
C    *ZR
     ...
     * ZNI, ZNR, ZR, ZRI, ZRR, D1MACH, ZSABS
...
C-----------------------------------------------------------------------
  ...
  110 CONTINUE
      ASCLE = 1.0D+3*D1MACH(1)/TOL
      CALL ZSLOG(PHIR, PHII, STR, STI, IDUM)
      CZR = CZR + STR
      CZI = CZI + STI
      IF (IFORM.EQ.1) GO TO 120
      CALL ZSLOG(ARGR, ARGI, STR, STI, IDUM)
...
C
C***ROUTINES CALLED  D1MACH,ZBKNU,ZRATI,ZSABS
...
C     COMPLEX CINU,CSCL,CT,CW,C1,C2,RCT,ST,Y,ZR
     ...
     * STI, STR, TOL, YI, YR, ZRI, ZRR, ZSABS, D1MACH
