      PROGRAM CAXSREFL3D
C--------/---------/---------/---------/---------/---------/---------/--
C                       qsub -q long -r rncaxsrefl
C
C     Check 
C           ynctsphr,Q0,DEFP,RMUF,REV,FCT,NUNIT,ICOMPL,ynrealeps
C     before running
C     Check ci lines and IPLP setting before running program !!!
C     Look at AXI if redundant???
C     rsnm and rev overlapping???
C     Assumes that the last component number with IT=2 is icompl=ncomp-1
C      if (ordered) then
C        if (icomp.eq.3) then            !!temporarily dmd option ON!!
C        call stack(nstack,ncompd,icomp,fab,nplan(icomp)+1,dl,dr,dist)
C     ------------------------------------------------------------------  
C     A B S T R A C T   
...
C  
      ...
      logical ynbrug,ordered,ynphase,ynperfcon,ynperfconv,ynctsphr,
     & ynrealeps,ynfct
      real*8 TOL,EPS0
...
*
* Unless ynrealeps=.false., band structure calculation is performed
* with only the real part of a material dielectric constant.
* This choice is only effective for KTYPE=3!!!
*    
      PARAMETER (ynrealeps=.true.)     
*     
* YNFCT controls the units of the output frequency ZVAL. If ynfct=.true. then
C ZVAL=2*PI*ALPHA/LAMBDA (I.E., FREQUENCY IS IN THE UNITS [C/ALPHA]).
C If ynfct=.false. then ZVAL=ZVAL_{3d}=PI*A/LAMBDA (I.E., FREQUENCY IS IN THE UNITS [2C/A]),
C where A=sqrt(2.d0)*ALPHA. The latter choice is used in the 3D band structure 
C calculations (BSC). For instance, for a FCC LATTICE, A is the side of a 
C conventional unit cell of a cubic lattice which has length A=2 and
C A=sqrt(2.d0)*ALPHA. Thus the former and latter ZVAL are related to OMEGA in BSC by 
C                             ZVAL=ZVAL_{3d}*sqrt(2.d0)
C The control via YNFCT is perforemd via numerical values of the constant FCT in routines
C SCAT.F and BAND.F
*
      PARAMETER (ynfct=.true.)     
*       
...
c      PARAMETER (CCEPS=(1.45D0,0.d0)**2)
      PARAMETER (CCEPS=(-1.D4,10.d0))
...
c Host of spheres (background) dielectric constant
      PARAMETER (EPS0=2.24D0)                  !1.4D0**2
c dielectric constant of the left host:
      PARAMETER (EPSL=(1.D0,0.d0))
...
C   C ..  SCALAR VARIABLES ..   C  
      ...
      INTEGER      IG0,NUNIT,ICOMP,ICOMPL,KEMB,IU,IP,IPL,IPLP,ILAYER
      INTEGER      IEPS,ISTEP,NSTEP,NBAS,ILCSN
      INTEGER      ICHOICE,NP,NCHECK,NAXSM,NDGS   
      REAL*8       ALPHA,EMACH,PI,EPSILON,XD,DIST  
...
C    
      ...
      REAL*8     S(NCOMPD,NPLAND),AL(2),D(NCOMPD),VEC0(3),AQ(2) 
...
* transfers integers NCHECK,NAXSM,NDGS from the main to TMTAXSP
*     
...
C ..  EXTERNAL ROUTINES ..  
C  
      EXTERNAL ELMGEN,LAT2D,PCSLAB,PCCSLAB,PCCSLABC,HOSLAB,PAIR,SCAT,
     & BAND,REDUCE  
C  
C ..  DATA STATEMENTS ..  
C  
...
C             of the resulting sphere cut by a plane on its top
C     NP=-5 - DEFP is the height (along the axial symmetry axis) 
C             of the resulting sphere cut by a plane on its bottom
...
cc      read(5,*) rev
*
      rev=250.d0                         !feeded as REV to RSP* routines
* 
      AXI=rev
*
...
* If specular RTA is to be calculated (decided by the parameter YNSP)
      ynspec=ynsp
*
* Redefine ALPHAP for the (110) fcc stacking, which consists of the stacking
* of rectangular planes

      if (nstack.eq.3) ALPHAP=SQRT(2.d0)      ! (110) fcc stacking
*
*
...
cc      AQ(1)=2.D0*PI*AQ(1)  
cc      AQ(2)=2.D0*PI*AQ(2)
...
**************************
* RMUF as a function of the filling fraction ff
*
c      ff=0.6d0
c      if (nstack.eq.0) 
c     & rmuf=(3.d0*ff/(4.d0*pi))**(1.d0/3.d0)*sqrt(2.d0)       !sc lattice 
c     rmuf=(3.d0*ff/pi)**(1.d0/3)/sqrt(2.d0)                   !bcc lattice
c      if (nstack.eq.1.or.nstack.eq.2)
c     &   rmuf=(3.d0*ff/(2.d0*pi))**(1.d0/3)/sqrt(2.d0)        !fcc lattice 
***************************
c      rmuf=0.6873059437985093d0/sqrt(2.d0)
c      rmuf=0.4999999999999999999d0
c      rmuf= 0.481186509d0                        !f_{fcc}=0.66
      rmuf=sqrt(6.d0)/8.d0                        !cp (111) diamond 
c      rmuf=sqrt(6.d0)/8.d0                        !cp (100) diamond 
c      rmuf = 418.d0/950.d0
*           rmuf*zval*sqrt(eps0) is the size parameter here !!!
* rsnm=rmuf*A(in nm)
*
c      write(6,*)'READ SPHERE RADIUS IN NM'
c      read(5,*) RSNM
      rsnm=rev
C=======================================================================  
...
*
      ...
      DATA IT(7)/2/
*
*  Specify the last component number with IT=2
* (Used later to avoid protruding spheres at the stack sides)
*
      icompl=ncomp-1                 
*
...
* 
* HOMOGENEOUS PLATE:
*    NOTE THE PRESENCE OF THE BLOCK AFTER THE END OF COMPONENTS
*    ASSIGNING PARAMETERS LOOP BELOW, WHICH CAN PARTLY OVERWRITTE
...
C--------/---------/---------/---------/---------/---------/---------/-- 
      if (ordered) then
        if (icomp.eq.3) then               !temporarily dmd option
...
**********************
* Correction for the facet to avoid protruding spheres
* at either side of a stack

      if (icomp.eq.2) then

* Specify for the first plane of the first component with IT=2:

      DL(1,2,1)=0.d0
      DL(2,2,1)=0.d0
      DL(3,2,1)=rmuf 

      else if (icomp.eq.icompl) then

* icompl is the last component number with IT=2 (specified above):
  
      DR(1,icompl,nplan(icompl))=0.d0
      DR(2,icompl,nplan(icompl))=0.d0
      DR(3,icompl,nplan(icompl))=rmuf   
     
      end if                       !icompl
* 
* Temporarily option for sphere cut by a plane termination
* in the case of a diamond lattice
*
      if ((nstack.eq.8).or.(nstack.eq.9).or.(nstack.eq.10)) then

       xd=(rmuf-dist/2.d0)/(rmuf+dist/2.d0)
       
        if (icomp.eq.2) then          !icomp

           if (defp.gt.xd) then       !defp

           do ip=1,1
ct            DL(1,icomp,ip)=DEFP*DL(1,icomp,ip)/2.d0
ct            DL(2,icomp,ip)=DEFP*DL(2,icomp,ip)/2.d0
              DL(3,icomp,ip)=DEFP*DL(3,icomp,ip)/2.d0
ct            DR(1,icomp,ip)=DEFP*DR(1,icomp,ip)/2.d0
ct            DR(2,icomp,ip)=DEFP*DR(2,icomp,ip)/2.d0
              DR(3,icomp,ip)=DEFP*DR(3,icomp,ip)/2.d0 
           enddo 
           end if                    !defp
*
...
*
         do ip=1,nplan(icomp)
            DL(1,icomp,ip)=DL(1,icomp,ip+1)
            DL(2,icomp,ip)=DL(2,icomp,ip+1)
            DL(3,icomp,ip)=DL(3,icomp,ip+1)
            DR(1,icomp,ip)=DR(1,icomp,ip+1)
            DR(2,icomp,ip)=DR(2,icomp,ip+1)
            DR(3,icomp,ip)=DR(3,icomp,ip+1)     
         enddo        
        end if                       !icomp
*  
      end if                         !nstack
*
**********************   END OF FACET CORRECTION  ***********************
...
c >>> band structure: determine parallel components of the \vd vector
C                     which takes one from a point in the nth slice
C                     to an equivalent point in the (n+1)th slice

      al(1)=0.d0
      al(2)=0.d0

cc      READ(10,*) DUMMY,(AL(I),I=1,3) 
*     NSTACK             stacking sequence
*        0               sc lattice in the (100) direction
*        1               fcc lattice in the (111) direction
*        2               fcc lattice in the (100) direction
...
C--------/---------/---------/---------/---------/---------/---------/-- 
cc      if (nstack.eq.0)  then
cc          al(3)=1.d0
cc      else 
      if ((nstack.eq.1).or.((nstack.ge.7).and.(nstack.le.9))) then
          al(1)=(1.d0+cos(pi/3.d0))/3.d0
          al(2)=sin(pi/3.d0)/3.d0
cc          al(3)=2.d0/sqrt(6.d0)
cc      else if ((nstack.eq.2).or.(nstack.eq.10))   then
cc          al(3)=sqrt(2.d0)
      end if        !nstack
...
*
*
      if (NSTACK.lt.0.or.NSTACK.gt.12) write(6,*)'Illegal value of 
     & NSTACK'
*
      if (ynsp.eq.'y') then
...
*  OUTPUT WRITING 
      ...
      OPEN(UNIT=50,FILE='caxsrefl3d.log')
      rewind(50)
      write(50,*)'#Calculation performed with LMAX=',lmax
      write(50,*)'#Calculation performed with RMAX=',rmax
      if (.not.ynfct) then
      write(50,*)'#FCT=SQRT(2) and ZVAL=2*PI*ALPHA/(FCT*LAMBDA)'
      write(50,*)'#ZVAL the same as in CCPATH'
cc      write(50,*)'#FCT=1 in BAND - different ZVAL as in CCPATH'
      end if
      if ((ktype.eq.3).and.(ynrealeps)) write(50,*)'#Only the REAL part 
     & of a material dielectric constant was used in the calculation!!!'
      write(50,*)'KTYPE=',KTYPE
      write(50,*)'ynsp=',ynsp
      write(50,*)'ynbrug=',ynbrug
      write(50,*)'ync=',ync
      write(50,*)'ynperfcon=',ynperfcon
      write(50,*)'ynrealeps=',ynrealeps
      write(50,*)'ynphase=',ynphase
      write(50,*)'ilcs=',ilcs
      write(50,*)'NMAT=',NMAT
      write(50,*)

      if (ktype.eq.3) goto 99

      OPEN(UNIT=NOUT,FILE='cex100-1uc.dat')    !cx9df1p9np4Xe2
      rewind(NOUT)
      write(nout,*)'#Calculation performed with LMAX=',lmax
      write(nout,*)'#Calculation performed with RMAX=',rmax
      if (ynfct) 
     &   write(nout,*)'#ZVAL=2*PI*ALPHA/LAMBDA; UNITS [c/ALPHA]'
      if (.not.ynfct) then
      write(nout,*)'#FCT=SQRT(2) and ZVAL=2*PI*ALPHA/(FCT*LAMBDA)'
      write(nout,*)'#ZVAL the same as in CCPATH'
cc      write(nout,*)'#FCT=1 in BAND - different ZVAL as in CCPATH'
      end if
C--------/---------/---------/---------/---------/---------/---------/-- 

      if ((ktype.eq.3).and.(ynrealeps))
     & write(nout,*)'#Only the REAL part of a material dielectric 
     & constant was used in the calculation!!!'
      write(nout,*)'#'
*
      ...
      IF(NP.EQ.-5) WRITE(NOUT,7180) DEFP
...
*
      ...
      if (NSTACK.eq.0) then 
      write(6,*)'sc lattice in the (100) direction'
      ...
      write(6,*)'fcc lattice in the (110) direction'
      write(nout,*)'#fcc lattice in the (110) direction'
     ...
     & as an fcc lattice'
      ...
      write(6,*)'the (100) stacking of a diamond lattice'
      write(nout,*)'#the (100) stacking of a diamond lattice'
      else if (NSTACK.eq.11) then 
      write(6,*)'AB2 lattice - indiv. layers'
      write(nout,*)'#AB2 lattice - indiv. layers'
      else if (NSTACK.eq.12) then 
...
CS      if ((lcs.eq.1).and.(nmat.eq.0)) 
CS     &     write(nout,*)'#sphere diel. const.=', zeps(1)  
CS      if (lcs.ge.2) then
CS      write(nout,*)'#core radius/sphere radius =',rff(1)
CS      if ((ilcs.ne.1).or.(nmat.eq.0))  
CS     &  write(nout,*)'#sphere core diel. const.=', zeps(1)  
CS      if ((ilcs.ne.lcs).or.(nmat.eq.0))  
CS     &  write(nout,*)'#coating diel. const.=',zeps(lcs)
C--------/---------/---------/---------/---------/---------/---------/-- 
CS      end if  
*
...
***********************************************************************      
      ...
      OPEN(UNIT=40,FILE='phex100-1uc.dat')      !px9df1p9np4Xe2
      rewind(40)
      write(40,*)'#Calculation performed with LMAX=',lmax
      write(40,*)'#Calculation performed with RMAX=',rmax
      if (ynfct) 
     &   write(40,*)'#ZVAL=2*PI*ALPHA/LAMBDA; UNITS [c/ALPHA]'
      if (.not.ynfct) then
      write(40,*)'#FCT=SQRT(2) and ZVAL=2*PI*ALPHA/(FCT*LAMBDA)'
      write(40,*)'#ZVAL the same as in CCPATH'
cc      write(40,*)'#FCT=1 in BAND - different ZVAL as in CCPATH'
      end if

      if ((ktype.eq.3).and.(ynrealeps)) write(40,*)'#Only 
     & the REAL part of a material dielectric constant was used
     & in the calculation!!!'
      write(40,*)'#'
*     
...
*
      ...
      write(40,*)'#fcc lattice in the (110) direction'
      else if (NSTACK.eq.4) then 
     ...
     & as an fcc lattice' 
      ...
      write(40,*)'#the (100) stacking of a diamond lattice'
      else if (NSTACK.eq.11) then 
...
C--------/---------/---------/---------/---------/---------/---------/--
      ...
      END IF
*
      if (nmat.ge.1) write(40,*)'#Dispersive layer number=', ilcs
CS      if ((lcs.eq.1).and.(nmat.eq.0)) 
CS     &     write(40,*)'#sphere diel. const.=', zeps(1)  
CS      if (lcs.ge.2) then
CS      write(40,*)'#core radius/sphere radius =',rff(1)
CS      if ((ilcs.ne.1).or.(nmat.eq.0))  
CS     &  write(40,*)'#sphere core diel. const.=', zeps(1)  
CS      if ((ilcs.ne.lcs).or.(nmat.eq.0))  
CS     &  write(40,*)'#coating diel. const.=',zeps(lcs)
C--------/---------/---------/---------/---------/---------/---------/-- 
CS      end if  
*
...
*********************************************************************** 
 ...
 99   OPEN(UNIT=nout+1,FILE='fccDimaXz1bndR.dat')
      rewind(nout+1)
      write(nout+1,*)'#Calculation performed with LMAX=',lmax
      write(nout+1,*)'#Calculation performed with RMAX=',rmax
      if (ynfct) 
     &  write(nout+1,*)'#ZVAL=2*PI*ALPHA/LAMBDA; UNITS [c/ALPHA]'
      if (.not.ynfct) then
      write(nout+1,*)'#FCT=SQRT(2) and ZVAL=2*PI*ALPHA/(FCT*LAMBDA)'
      write(nout+1,*)'#ZVAL the same as in CCPATH'
cc      write(nout+1,*)'#FCT=1 in BAND - different ZVAL as in CCPATH'
      end if
      if (ynrealeps) write(nout+1,*)'#Only the REAL part of a material 
     & dielectric constant used in the band structure calculation!!!'
C--------/---------/---------/---------/---------/---------/---------/--
*
      write(nout+1,*)'#'
      WRITE(nout+1,*)'#In-plane wave vector AQ(1),AQ(2)=',AQ(1),AQ(2) 
c      WRITE(nout+1,*)'#Unit slice width in program units=', AL(3) 
      write(nout+1,*)'#'
      
      WRITE(nout+1,5454) ICHOICE,NCHECK
      IF(NP.EQ.-1.AND.DEFP.GE.1D0) WRITE(nout+1,7000) DEFP
      IF(NP.EQ.-1.AND.DEFP.LT.1D0) WRITE(nout+1,7001) DEFP
      IF(NP.GE.0) WRITE(nout+1,7100) NP,DEFP
      IF(NP.EQ.-2.AND.DEFP.GE.1D0) WRITE(nout+1,7150) DEFP
      IF(NP.EQ.-2.AND.DEFP.LT.1D0) WRITE(nout+1,7151) DEFP
      IF(NP.EQ.-3) WRITE(nout+1,7160)
      IF(NP.EQ.-4) WRITE(nout+1,7170) DEFP
      IF(NP.EQ.-5) WRITE(nout+1,7180) DEFP
      WRITE(nout+1,7200) DDELT
      IF (DABS(RAT-1D0).LE.1D-6) WRITE(nout+1,8003) AXI
      IF (DABS(RAT-1D0).GT.1D-6) WRITE(nout+1,8004) AXI
*
      if (ordered) then
       write(nout+1,*)'#Ordered structure'
      if (NSTACK.eq.0) then 
      write(nout+1,*)'#sc lattice in the (100) direction'
      else if (NSTACK.eq.1) then 
       write(nout+1,*)'#fcc lattice in the (111) direction'
      else if (NSTACK.eq.2) then 
       write(nout+1,*)'#fcc lattice in the (100) direction'
      else if (NSTACK.eq.3) then 
       write(nout+1,*)'#fcc lattice in the (110) direction'
      else if (NSTACK.eq.4) then 
       write(nout+1,*)'#hcp lattice in the (111) direction'
      else if (NSTACK.eq.5) then 
      write(nout+1,*)'#bct lattice in the (100) direction'
      else if (NSTACK.eq.6) then 
      write(nout+1,*)'#hcp layers stacked on top of each other'
      else if (NSTACK.eq.7) then 
      write(nout+1,*)'#diamond (111) as stacking of complex bilayers'
      else if (NSTACK.eq.8) then 
      write(nout+1,*)'#AABBCC stacking of the (111) diamond
     &  as an fcc lattice'
      else if (NSTACK.eq.9) then 
      write(nout+1,*)'#ABBCCA stacking of the (111) diamond
     & as an fcc lattice'
      else if (NSTACK.eq.10) then 
      write(nout+1,*)'#the (100) stacking of a diamond lattice'
      else if (NSTACK.eq.11) then 
      write(nout+1,*)'#AB2 lattice - indiv. layers'
      else if (NSTACK.eq.12) then 
      write(nout+1,*)'#AB2 lattice - complex bilayers'
      end if
      else
      write(nout+1,*)'#Disordered structure'
      end if
*
C--------/---------/---------/---------/---------/---------/---------/--
*
      if (polar.eq.'S ') then
         write(nout+1,*)'#TE or s-polarization'
      else if (polar.eq.'P ') then
         write(nout+1,*)'#TM or p-polarization'
      end if
*      write(nout+1,*)'#Filling fraction ff=cp'
c, ff
      write(nout+1,*)'#Sphere radius in nm=', rsnm
      write(nout+1,*)'#Rel. sphere radius=', rmuf
      if (ync.eq.'n') then
        write(nout+1,*)'#Homogeneous spheres'
      else if (ync.eq.'y') then
       write(nout+1,*)'#n_s=1.45d0 coated spheres'
c* either
c       write(nout+1,*)'#Spheres of Ag core-n_s=2 shell, rff=0.38'
* or
       write(nout+1,*)'#Sphere 1: Ag core with rff=0.75;
     &  Sphere 2: eps=1.45d0**2, rff=1'
*
      end if 
      write(nout+1,*)'#host dielectric constant=', eps0
C--------/---------/---------/---------/---------/---------/---------/--
      IF(.NOT.YNPERFCON) THEN
       write(nout+1,*)'#Material number =',NMAT
      ELSE
       write(nout+1,*)'#Perfect conductor'
      END IF
*
      if (nmat.ge.1) write(nout+1,*)'#Dispersive layer number=', ilcs
CS      if ((lcs.eq.1).and.(nmat.eq.0)) 
CS     &     write(nout+1,*)'#sphere diel. const.=', zeps(1)  
CS      if (lcs.ge.2) then
CS      write(nout+1,*)'#core radius/sphere radius =',rff(1)
CS      if ((ilcs.ne.1).or.(nmat.eq.0))  
CS     &  write(nout+1,*)'#sphere core diel. const.=', zeps(1)  
CS      if ((ilcs.ne.lcs).or.(nmat.eq.0))  
CS     &  write(nout+1,*)'#coating diel. const.=',zeps(lcs)
C--------/---------/---------/---------/---------/---------/---------/-- 
CS      end if  
*
      if (ynbrug) write(nout+1,*)'#Bruggeman approximation performed'
*
      write(nout+1,*)'#NUNIT=', NUNIT
      write(nout+1,*)'#NCOMP=', NCOMP
      do icomp=1,ncomp
      write(nout+1,*)'#ICOMP=', ICOMP
      write(nout+1,*)'#NLAYER=', NLAYER(ICOMP),',',
     &             ' NPLAN=',NPLAN(ICOMP)
        IF((IT(ICOMP).EQ.1).and.(D(icomp).ne.0)) THEN 
          write(nout+1,*)'#D(icomp)=',D(icomp)
          write(nout+1,*)'#EPS2(icomp)=',EPS2(icomp) 
        END IF      
      if (.not.ordered) write(nout+1,*)'#ISTACK=',ISTACK(ICOMP)
      enddo
      WRITE(nout+1,*)'#ZVAL in columns'
      write(nout+1,*) 
...
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF=1.4d0
c      ZINF=0.15d0    !2.d0*pi*rsnm/(3600.d0*rmuf)  !1.469923168
      ZSUP=3.5d0
c      ZSUP=1.5d0    !2.d0*pi*rsnm/(450.d0*rmuf)   !3.322821292
...
*  regular spheres. Later it will be useful to change ynctsphr,
*  (together with NP, DEFPP) into an array which will assign
*  the type of a scatterer for each scattering plane
*
      ynctsphr=.false.                      !.true.
*                  
...
* ZVAL=2*PI*ALPHA/LAMBDA ===> lambda=lambda*(rsnm/A)/rmuf=lambda
      ...
      if ((nmat.eq.0).or.(ynperfcon)) goto 80           !dispersionless dielectric 
                                                        !or ideal metal
      ...
      DO ICOMP=1,NCOMP 
       DO IPL=1,NPLAN(ICOMP) 
        ...
        if(it(icomp).eq.2) then
          ...
          if ((ktype.eq.3).and.(ynrealeps)) zeps1=dble(zeps1)

          EPSSPH(icomp,ipl)=zeps1
ci          if (mod(ipl,2).eq.1) EPSSPH(icomp,ipl)=zeps1
ci          if (mod(ipl,2).eq.0) EPSSPH(icomp,ipl)=1.45d0**2
        endif
       enddo
      enddo
******
...
*
c      IF (NBAS.EQ.1) THEN
c      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(1),EPSSPH(1,1),MU1(1)  
c     &        ,MUSPH(1,1),KAPPA,AK,DL(1,1,1),DR(1,1,1),G,ELM,A0,EMACH,  
c     &		 QIL,QIIL,QIIIL,QIVL)  
c      ELSE
      IPLP=1
...
*
c      IF (NBAS.EQ.1) THEN
c      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(1),EPSSPH(1,IPL),MU1(1),  
c     &           MUSPH(1,IPL),KAPPA,AK,DL(1,1,IPL),DR(1,1,IPL),  
c     &           G,ELM,A0,EMACH,QIR,QIIR,QIIIR,QIVR) 
c      ELSE
      CALL PCCSLAB(YNC,LMAX,IGMAX,NBAS,RAP,EPS1(1),EPSSPH(1,IPL),MU1(1),  
...
*
c      IF (NBAS.EQ.1) THEN
c      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(ICOMP),EPSSPH(ICOMP,1)  
c     &          ,MU1(ICOMP),MUSPH(ICOMP,1),KAPPA,AK,DL(1,ICOMP,1),  
c     &           DR(1,ICOMP,1),G,ELM,A0,EMACH,QIR,QIIR,QIIIR,QIVR) 
c      ELSE
...
*
*      
      if (ynctsphr) then       !if sphere cut by a plane
     ...
     &           DR(1,ICOMP,1),G,A0,EMACH,QIR,QIIR,QIIIR,QIVR)
      ...
      else                    !if regular sphere
      
      CALL PCCSLAB(YNC,LMAX,IGMAX,NBAS,RAP,EPS1(ICOMP),EPSSPH(ICOMP,1)  
     &          ,MU1(ICOMP),MUSPH(ICOMP,1),KAPPA,AK,DL(1,ICOMP,1),  
     &           DR(1,ICOMP,1),G,A0,EMACH,QIR,QIIR,QIIIR,QIVR)


      end if                  !end of cut sphere decision tree
...
*
c      IF (NBAS.EQ.1) THEN
c      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(ICOMP),EPSSPH(ICOMP,IPL),  
c     &           MU1(ICOMP),MUSPH(ICOMP,IPL),KAPPA,AK,  
c     &           DL(1,ICOMP,IPL),DR(1,ICOMP,IPL),G,ELM,A0,EMACH,  
c     &           QIR,QIIR,QIIIR,QIVR)
c      ELSE                    !NBAS.GT.1
      CALL PCCSLAB(YNC,LMAX,IGMAX,NBAS,RAP,EPS1(ICOMP),  
...
*
   ...
   16 CONTINUE  
    ...
    4 CONTINUE  
                                                                   ...
                                                                   ENDIF  
******************************************************
      IF(KTYPE.LT.3) THEN                !main calculation switch
C  
   ...
   11        CONTINUE  
     ...
     &                   VEC0,0.D0,QIL,QIIL,QIIIL,QIVL,EMACH)  
	     CALL PAIR(IGKMAX,QIL,QIIL,QIIIL,QIVL,QIR,QIIR,QIIIR,QIVR)  
           ...
           CALL SCAT(YNPHASE,YNFCT,IGMAX,ZVAL,AK,G,DBLE(KAPIN),
     &                 DBLE(KAPOUT),EINCID,QIL,QIIIL) 
* 
                     ELSE                    !KTYPE=3
C   
...
C  
     ...
     &    STOP 'IMPROPER MATCHING OF SUCCESSIVE HOST MEDIA' 
*
      CALL BAND(YNFCT,IGMAX,ZVAL,EMACH,AK,G,AL,KAPL,KAPR,
     &          QIL,QIIL,QIIIL,QIVL) 
*
                     ENDIF                    !KTYPE switch
*
...
*--------/---------/---------/---------/---------/---------/---------/--
 ...
 7170 FORMAT('#SHERE CUT BY A PLANE ON ITS TOP, DEFP=H/REV=',F11.7)
 7180 FORMAT('#SHERE CUT BY A PLANE ON ITS BOTTOM, DEFP=H/REV=',F11.7)
...
c  220 FORMAT(3X,'EPS:',2F10.5,' | ',4(3X,2F10.5))  
  ...
  229 FORMAT(3X,'THE SAMPLE HAS',I6,' PLANES IN THE',I6,'TH COMPONENT')
         END  
C======================================================================= 
      SUBROUTINE SCAT(YNPHASE,YNFCT,IGMAX,ZVAL,AK,G,KAPIN,KAPOUT,EINCID,
     &                QI,QIII) 
      IMPLICIT NONE 
C     ------------------------------------------------------------------  
...
C 
C     ynphase=.true.      ! true only if reflection phase is required           
C     ------------------------------------------------------------------  
C  
C ..  PARAMETER STATEMENTS ..  
C 
      INTEGER   IGD,IGKD
      logical ynphase,ynfct 
      PARAMETER (IGD=37,IGKD=2*IGD)  
C  
C ..  SCALAR ARGUMENTS  ..  
C  
      INTEGER    IGMAX  
      REAL*8     ZVAL,KAPIN,KAPOUT,XX,FCT   
C  
C ..  ARRAY ARGUMENTS ..  
C  
      REAL*8     AK(2),G(2,IGD)  
...
*
      if (ynfct) then
       ...
       FCT=SQRT(2.d0)     !To rescale ZVAL to CCPATH units
      end if
*
      DOWN=0.D0  
      REFLE=0.D0  
...
*
      ...
      write(6,*) 'ZVAL, PHASE=',   ZVAL, PHASE   
      write(40,*) ZVAL/FCT, PHASE   !,
...
c      pause
      END IF
*
      WRITE(8,101)  ZVAL/FCT,TRANS,REFLE,ABSOR  
...
*
   ...
   15 CONTINUE 
******************************************************* 
C           !temporarily option

      XX=EREFLE(IGK1)*DCONJG(EREFLE(IGK1))*GKZIN 

      if(XX.gt.1.d-4) then           !temporarily option
      write(67,*) 'IG1=', IG1 
      write(67,*)  sqrt(TES1),G(1,IG1),G(2,IG1)
      write(67,*)  XX
      end if
c 
*******************************************************
...
cb      call gzbsvd3d(IGKMAX,IGKD,W2,QINV1(1,IGK2),emach) 
cb      call gzbsvd3d(IGKMAX,IGKD,W3,QINV2(1,IGK2),emach) 
*
    4 CONTINUE  
      DO 5 IGK1=1,IGKMAX  
      DO 5 IGK2=1,IGKMAX  
...
C  
   ...
   14 CONTINUE 
      ...
      END  
C=======================================================================  
      SUBROUTINE BAND(YNFCT,IGMAX,ZVAL,EMACH,AK,G,AL,KAPL,KAPR,  
     &                QI,QII,QIII,QIV) 
...
C  
      INTEGER    IGMAX  
      REAL*8     ZVAL,EMACH,FCT  
      COMPLEX*16 KAPL,KAPR  
      LOGICAL ynfct
C  
C ..  ARRAY ARGUMENTS ..  
C  
      REAL*8     AK(2),G(2,IGD),AL(2)  
...
C*****DEPEND STRONGLY ON IGMAX.  
   ...
   16 CONTINUE 
      ...
      WRITE(9,101)  ZVAL/FCT,AKZREP(1),AKZREN(LN-1) 
      WRITE(9,103)  AKZIMP(1),AKZIMN(LN-1)   
      ELSE  
      WRITE(6,101)  ZVAL,(AKZAP(I),I=1,LU-1)  
      WRITE(9,101)  ZVAL/FCT,(AKZAP(I),I=1,LU-1)  
      END IF  
      RETURN  
