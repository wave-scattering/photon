...
C ..  DATA STATEMENTS ..  
C  
      DATA PI/3.14159265358979D0/,EMACH/1.d-10/,EPSILON/0.D0/  
...
cc      rmuf= 0.481186509d0                        !f_{fcc}=0.66
...
*
      ...
      if (ynbrug.eq.'y') write(6,*)'Bruggeman approx. used!'
      end if
*
C======================================================================= 
*  OUTPUT WRITING 
      OPEN(UNIT=NOUT,FILE='rnrefllm9zhs2ucx.dat')
      rewind(NOUT)
      write(nout,*)'#Calculation performed with LMAX=',lmax
      write(nout,*)'#Calculation performed with RMAX=',rmax
      write(nout,*)'#Matrix inversion using NAG routines' 
*
      if (ordered) then
...
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF=2.d0*pi*rsnm/(4000.d0*rmuf)
...
*
  ...
  49      CONTINUE  
         ALM=ALM/KAPPA  
         IF(I-J)51,50,51  
  50     ALM=ALM+CI            !adds CI to diagonal
  51     IF(II)52,52,53  
  52     XODD(J,I)=CI*ALM 
         GOTO 54  
  53     XEVEN(J,I)=CI*ALM  
...
C  
    ...
    2 CONTINUE 
*
cs      CALL ZGE(W2,INT,IGKMAX,IGKD,EMACH)  
cs      CALL ZGE(W3,JNT,IGKMAX,IGKD,EMACH) 
* 
      DO 4 IGK2=1,IGKMAX  
*
cs      CALL ZSU(W2,INT,QINV1(1,IGK2),IGKMAX,IGKD,EMACH)  
cs      CALL ZSU(W3,JNT,QINV2(1,IGK2),IGKMAX,IGKD,EMACH)
...
cs      CALL ZGE(XXMAT1,INT1,LMTOT,LMTD,EMACH)  
cs      CALL ZGE(XXMAT2,INT2,LMTOT,LMTD,EMACH)  
      ISIGN2=1  
      ...
      BMEL2(IOD)=TH(L+1)*AH(K2,II+1)  
                             END IF  
    2 CONTINUE  
cs      CALL ZSU(XXMAT1,INT1,BMEL1,LMTOT,LMTD,EMACH) 
      call gzbsvd3d(lmtot,lmtd,xxmat1,bmel1,emach)  
cs      CALL ZSU(XXMAT2,INT2,BMEL2,LMTOT,LMTD,EMACH) 
      call gzbsvd3d(lmtot,lmtd,xxmat2,bmel2,emach)  
      DO 4 ISIGN1=1,2  
