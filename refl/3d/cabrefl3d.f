...
*        6               diamond (111) as stacking of complex bilayers
*        7               diamond (111) as fcc - AABBCC stacking
*        8               diamond (111) as fcc - ABBCCA stacking
*        9               AB2 lattice - indiv. layers
*       10               AB2 lattice - single complex bilayer
* 
      PARAMETER (NSTACK=9)
*
...
C   C ..  SCALAR VARIABLES ..   C  
      ...
      INTEGER      IG0,NUNIT,ICOMP,KEMB,IU,IPL,IPLP,ILAYER,ILCSN
...
c      rmuf=sqrt(6.d0)/8.d0                        !cp diamond 
...
* RSNM is the sphere radius in nm. 
      D(1)=0.8d6/(2.d0*rsnm)   !True only for close-packed lattices
c      D(1)=0.d0
      D(3)=0.D0
* reference vectors
c       DL(1,icomp,1)=0.d0
...
C======================================================================= 
*  OUTPUT WRITING 
      OPEN(UNIT=NOUT,FILE='rncabreflw2x.dat')
      rewind(NOUT)
...
* either
       write(nout,*)'#Spheres of Ag core-n_s=2 shell, rff=0.4'
* or
c       write(nout,*)'#Sphere 1: Ag core with rff=0.75;
c     &  Sphere 2: eps=1.45**2, rff=1.'
*
      end if 
...
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF=2.d0*pi*rsnm/(489.d0*rmuf)
      ZSUP=2.d0*pi*rsnm/(400.d0*rmuf)
...
*
      write(6,*)
      write(6,*)'Collect RTA data from file rncabrefl.dat'
***********************
      close(nout)
      STOP  
...
C  
    ...
    2 CONTINUE 
*
cx      CALL ZGE(W2,INT,IGKMAX,IGKD,EMACH)  
cx      CALL ZGE(W3,JNT,IGKMAX,IGKD,EMACH)  
*
      DO 4 IGK2=1,IGKMAX 
* 
cx      CALL ZSU(W2,INT,QINV1(1,IGK2),IGKMAX,IGKD,EMACH)  
cx      CALL ZSU(W3,JNT,QINV2(1,IGK2),IGKMAX,IGKD,EMACH)  
      call gzbsvd3d(IGKMAX,IGKD,W2,QINV1(1,IGK2),emach) 
      call gzbsvd3d(IGKMAX,IGKD,W3,QINV2(1,IGK2),emach) 
*
    4 CONTINUE  
      DO 5 IGK1=1,IGKMAX  
      DO 5 IGK2=1,IGKMAX  
...
C  
      ...
      PARAMETER (LMAXD=7,LMAX1D=LMAXD+1,LMODD=(LMAXD*LMAX1D)/2)  
      PARAMETER (LM1SQD=LMAX1D*LMAX1D,IGD=37,IGKD=2*IGD,NELMD=7680)  
