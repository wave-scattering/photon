      PROGRAM REFL3D N7
* revised version by A. Moroz 28 03 2001
C--------/---------/---------/---------/---------/---------/---------/--
      IMPLICIT NONE  
C     ------------------------------------------------------------------  
C     A B S T R A C T   
...
C  
      ...
      logical ynbrug,ordered
      real*8 EPS0
      COMPLEX*16 CCEPS,CSEPS
...
*        3               hcp lattice in the (111) direction
*        4               bct lattice in the (100) direction
*        5               hcp layers stacked on top of each other
* 
      PARAMETER (NSTACK=1)
*
...
c          Au*new.dat             NFIN=142
      PARAMETER (NFIN=73) 
c If ynbrug=.true., performs Bruggeman approximation for ZEPS1
...
c sphere (core) dielectric constant  (depending whether ync is 'y' or 'n')   
      PARAMETER (CCEPS=(2.1025D0,0.d0))
C >>>     SPHERE (OUTER SHELL SCATTERER) PERMITTIVITY                  <<<
*  n(silica)=1.45  <--->    EPS(1)=2.1025D0
*  n(ZnS)=2.       <--->    EPS(1)=4.D0

      PARAMETER (CSEPS=(2.1025D0,0.d0))
c Host (background) dielectric constant
      PARAMETER (EPS0=1.D0**2)
...
C   C ..  SCALAR VARIABLES ..   C  
      ...
      INTEGER      IG0,NUNIT,ICOMP,KEMB,IU,IPL,ILAYER,ILCSN
      INTEGER      IEPS,ISTEP,NSTEP
...
C  
      ...
      REAL*8     RSNM,LAMBDA,FF,FILFRAC,rff
...
*
      ...
      IF(KTYPE.EQ.3) WRITE(6,225) AQ(1),AQ(2) 
              ELSE           ! KTYPE=1
c      READ(10,204) THETA,FI,POLAR,FEIN  
      DATA THETA/0.d0/
      DATA FI/0.d0/
...
* material code number KV see above
c	write(6,*)
c	write(6,*)' material code number' 
c	write(6,*)'   0             dispersionless dielectric'                              
c	write(6,*)'   1             Drude metal'
c	write(6,*)'   2             Ag'
c	write(6,*)'   3             Au'
c	write(6,*)'   4             ZnS '
c      write(6,*)'	Choose the material from above'
c      read (5,*) NMAT
*
...
**KV************** Input values *******************************
	write(6,*)
      write(6,*)'READ CORE-TO-SHELL RATIO'
      read (5,*) rff
c	rff = 0.71875
	write(6,*)
	write(6,*)
	if (NMAT.EQ.4) then
		write(6,*)'READ FILLING FRACTION OF ZnS IN THE ZnS-SILICA CORE'
      read (5,*) FILFRAC
c	filfrac = 0.62
	end if
	write(6,*)
      write(6,*)'READ SPHERE RADIUS IN NM'
      read (5,*) RSNM
c      rsnm= 128.d0
C=======================================================================  
...
* RSNM is the sphere radius in nm. 
      D(1)=0.85d6/(2.d0*rsnm)   !True only for close-packed lattices
c      D(1)=0.d0
      D(3)=0.D0
* reference vectors
       DL(1,icomp,1)=0.d0
...
* X axis oriented along the primitive vector of unit length of the 2d lattice.
      ...
      if (ordered) then
        call stack(nstack,ncompd,icomp,fab,nplan(icomp),dl,dr)
      else
        call stack(istack(icomp),ncompd,icomp,fab,nplan(icomp),dl,dr)
      end if
*
...
*
      if ((ync.eq.'y').and.(ilcs.gt.1)) then
      write(6,*)'YNC=', ync,' and ILCS=',ilcs,' params incompatible'
      stop
      end if
*
      if (nmat.gt.1) write(6,*)'Real material data are to be
     & provided'
C======================================================================= 
*  OUTPUT WRITING
*kv	file name input
	write(6,*)
      write(6,*)'READ OUTPUT FILE NAME (like NAME.DAT)'
      read (5,*) FILENAME 
	OPEN(UNIT=NOUT,FILE=FILENAME)
c
c
c      OPEN(UNIT=NOUT,FILE='rnrefl.dat')
      rewind(NOUT)
      WRITE(NOUT,*)'#LAMBDA, ZVAL, TRANS, REFL, ABSOR in columns'
...
c, ff
      write(nout,*)'#Sphere radius in nm=', SNGL(rsnm)
      write(nout,*)'#Rel. sphere radius=', SNGL(rmuf)
      if (ync .eq.'n') then
        write(nout,*)'#Homogeneous spheres'
      else if (ync.eq.'y') then
       write(nout,*)'#coated spheres'
       write(nout,*)'#rff=',SNGL (rff)
      end if 
      write(nout,*)'#host dielectric constant=', SNGL(eps0)
C--------/---------/---------/---------/---------/---------/---------/--
      write(nout,*)'#Material number =',nmat
      write(nout,*)'#The sphere component with material eps=',ilcs
*
...
* Execution
* 
* Assignement of common variable:
*
      ilcsn=ilcs
      ccepsn=cceps
      csepsn=cseps
...
c                     lambda=2.d0*pi*rsnm/(zval*rmuf)
      ZINF=2.d0*pi*rsnm/(2000.d0*rmuf)
      ZSUP=2.d0*pi*rsnm/(350.d0*rmuf)
...
* Sphere optical constants in the case of a dispersion:
      ...
      if (nmat.le.1) goto 52       ! goto frequency loop

C  else  > READING IN MATERIAL  DATA:
...
*
c      if (nmat.eq.3) go to 1
c      OPEN(UNIT=30,FILE='agc.dat')            !silver data
c      rewind(30)
c        do ieps=1,nfin
c          read(30,*) omf(ieps),ceps1(ieps)
c        enddo
c       close(30)
c       go to 52
c*
...
c      OPEN(UNIT=30,FILE='Audat.dat')          !Gold data in nm
c      write(6,*)'Gold particles'
c      rewind(30)
c        do ieps=1, nfin
c          read(30,*) omf(ieps),ceps1(ieps)
c          omf(ieps)=2.d0*pi*rsnm*omf(ieps)/(1240.d0*rmuf)
c          omf(ieps)=2.d0*pi*rsnm/(omf(ieps)*rmuf)
c        enddo
c       close(30)
*********************
 52   ZVAL=ZINF-ZSTEP  
...
* where lambda_z is the wavelength for which Re eps_s=0.
       ...
       reepsz=2.d0*pi*RSNM/(324.269d0*rmuf)

      if (nmat.ne.1) goto 71         ! not a Drude metal
      plasma=reepsz
        omxp=plasma/omega
 ...
 71   continue
      ...
      if (nmat.eq.4) then
*
c       filfrac=0.62d0         ! filfrac of ZnS in ZnS core
...
* If spheres NMAT.ne.0
      DO ICOMP=1,NCOMP 
       DO IPL=1,NPLAN(ICOMP)  
        if(it(icomp).eq.2) EPSSPH(icomp,ipl)=zeps1
       enddo
      enddo
******
  80  continue
...
*--------/---------/---------/---------/---------/---------/---------/--
      ...
      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(1),EPSSPH(1,IPL),MU1(1),  
   ...
   14 CONTINUE  
     ...
     &        0.D0) STOP 'IMPROPER MATCHING OF SUCCESSIVE HOST MEDIA' 
      ...
      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(ICOMP),EPSSPH(ICOMP,1)  
...
*--------/---------/---------/---------/---------/---------/---------/--
   ...
   17        CONTINUE  
      ...
      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(ICOMP),EPSSPH(ICOMP,IPL),  
...
*
      write(6,*)
      write(6,*)'Collect RTA data from file rnrefl.dat'
***********************
      close(nout)
      STOP  
...
C  
      ...
      REAL*8     DOWN,REFLE,TRANS,ABSOR,GKZIN,GKZOUT,TES1,LAMBDA  
      COMPLEX*16 CZERO  
      character*1 ynspec
C  
C ..  LOCAL ARRAYS  ..  
...
c      LEIN=.FALSE.
      ...
      IGK1=0 
*
      if((ynspec.eq.'y').or.(ynspec.eq.'Y')) then
       IG1=IG0
       go to 2     ! ignore do 1 loop
      end if
*
      DO 10 IG1=1,IGMAX  
 2    TES1=(AK(1)+G(1,IG1))*(AK(1)+G(1,IG1))+(AK(2)+G(2,IG1))*(AK(2)+  
     &            G(2,IG1))  
      GKZIN =0.D0  
      GKZOUT=0.D0
*
...
*
      ...
      pause
      END IF
      WRITE(8,101)  LAMBDA,ZVAL,TRANS,REFLE,ABSOR  
      WRITE(6,101)  LAMBDA,ZVAL,TRANS,REFLE,ABSOR  
      RETURN  
C  
  101 FORMAT(5E14.6)  
      END  
...
*  reflection and transmission coefficients of the slab:
      R(J,1)=(GKKZ1-X(J)*X(J)*GKKZ2)/DENOMA   ! z-component
      R(J,2)=           (GKKZ1-GKKZ2)/DENOMB  ! y-component
      T(J,1)=CTWO*X(J)*GKKZ1/DENOMA           ! a superficial X(j) here???
...
C  
      ...
      PARAMETER(LMAXD=7,LMAX1D=LMAXD+1,LM1SQD=LMAX1D*LMAX1D)  
C  
C ..  SCALAR ARGUMENTS  ..  
C  
      INTEGER    LMAX  
...
C  
    ...
    1 DLMH(K,1)=CZERO 
     ...
     &         +ALPHA*CT*DCONJG(CF)*Z1) 
      DLMH(2,II)=COEF*CI*(BETA*CF*Z2-ALPHA*DCONJG(CF)*Z1) 
...
C  
    ...
    3 AH(K,1)=CZERO  
      ...
      I=L*L+L-M+1  
      Z3=CC1*DBLE(M)*YLM(I)  
      AE(1,II)= SIGNUS*CI*(CF*Z1-DCONJG(CF)*Z2) 
      AE(2,II)=-SIGNUS*(CT*CF*Z1+ST*Z3+CT*DCONJG(CF)*Z2) 
      AH(1,II)= SIGNUS*(CT*CF*Z1+ST*Z3+CT*DCONJG(CF)*Z2) 
...
C  
      ...
      PARAMETER (LMAXD=7,LMAX1D=LMAXD+1,LMODD=(LMAXD*LMAX1D)/2)  
...
*
  ...
  23  CONTINUE 
*
c      TEST=ABS((TEST2-TEST1)/TEST1) 
c      TEST1=TEST2  
c      IF(TEST-0.001D0)27,27,24  
c  24  IF(N1-10)9,25,25  
c  25  WRITE(16,26)N1  
c  26  FORMAT(29H**DLM1,S NOT CONVERGED BY N1=,I2)  
c      GOTO 285  
c  27  WRITE(16,28)N1  
c  28  FORMAT(25H DLM1,S CONVERGED BY N1 =,I2)  
C     WRITE(16,250)DLM  
...
c      TEST=ABS((TEST2-TEST1)/TEST1)  
c      TEST1=TEST2  
c      IF(TEST-0.001D0)45,45,42  
c  42  IF(N1-10)32,43,43  
c  43  WRITE(16,44)N1  
c  44  FORMAT(31H0**DLM2,S NOT CONVERGED BY N1 =,I2)  
c      GOTO 465  
c  45  WRITE(16,46)N1  
c  46  FORMAT(24H DLM2,S CONVERGED BY N1=,I2)  
C  
...
C  
      ...
      PARAMETER (LMAXD=7,LMAX4D=4*LMAXD+2)  
C  
C ..  SCALAR ARGUMENTS  ..  
C  
...
C  
      ...
      PARAMETER (LMAXD=7,LMAX1D=LMAXD+1,LMODD=(LMAXD*LMAX1D)/2)  
      PARAMETER (LM1SQD=LMAX1D*LMAX1D,IGD=31,IGKD=2*IGD,NELMD=7680)  
...
C  
      ...
      PARAMETER(IGD=31,IGKD=2*IGD,IGK2D=2*IGKD)  
C  
C ..  SCALAR ARGUMENTS  ..  
C  
      INTEGER    IGMAX  
