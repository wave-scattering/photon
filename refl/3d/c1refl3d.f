...
C    
      ...
      REAL*8     RSNM,RCORE,LAMBDA,FF,FILFRAC
...
c      rsnm=250.d0
*
      write(6,*)'READ SPHERE CORE RADIUS IN NM'
      read(5,*) RCORE
...
c      ZINF=0.8d0
      ZINF=2.d0*pi*rsnm/(3600.d0*rmuf)  
c      ZSUP=1.6d0
      ZSUP=2.d0*pi*rsnm/(450.d0*rmuf)  
...
*
      CALL PCSLAB(YNC,LMAX,IGMAX,RAP,EPS1(ICOMP),  
...
c  220 FORMAT(3X,'EPS:',2F10.5,' | ',4(3X,2F10.5))  
  ...
  229 FORMAT(3X,'THE SAMPLE HAS',I6,' PLANES IN THE',I6,'TH COMPONENT')
         END  
C=======================================================================  
      subroutine stack(nstack,ncompd,icomp,fab,nplan,dl,dr)
C--------/---------/---------/---------/---------/---------/---------/--
c >>> nstack,icomp
C <<< fab,nplan,dl,dr
...
C--------/---------/---------/---------/---------/---------/---------/--      
      ...
      real*8 dl(3,ncompd,nplan),dr(3,ncompd,nplan),pi,fab,gamma
...
* Decision tree:
      ...
      go to 50 
      else if (nstack.eq.7) then  ! (111) diamond as fcc - AABBCC stacking
      fab=60.d0
        if (nplan.gt.6) then  
        write(6,*)'Stop in STACK.F'
      ...
      go to 70 
      else if (nstack.eq.8) then  ! (111) diamond as fcc - ABBCCA stacking
      fab=60.d0
        if (nplan.gt.6) then  
        write(6,*)'Stop in STACK.F'
        write(6,*)'NPLAN for fcc-diamond  .leq. 6'
...
*************************************************
 5    continue
*
* sc lattice in the (100) direction:
      DL(1,icomp,1)=0.d0
      DL(2,icomp,1)=0.d0
...
*************************************************
 10   continue
*
* fcc lattice in the (111) direction:
...
*
 20   continue
*
* fcc lattice in the (100) direction:
      DL(1,icomp,1)=0.d0
      DL(2,icomp,1)=0.d0
...
*************************************************
 30   continue
*
* hcp lattice in the (111) direction
      DL(1,icomp,1)=0.d0
      DL(2,icomp,1)=0.d0
      DL(3,icomp,1)=1.d0/sqrt(6.d0)
      if (nplan.gt.1) then
...
*************************************************
 40   continue
*
* bcc lattice in the (100) direction:
...
*
 50   continue
*
* hcp layers stacked on top of each other
      DL(1,icomp,1)=0.d0
      DL(2,icomp,1)=0.d0
      DL(3,icomp,1)=0.5d0
      DR(1,icomp,1)=0.d0
...
*************************************************
 70   continue
*
* diamond (111) as fcc - AABBCC stacking        !nstack.eq.7
*
      DL(1,icomp,1)=(1.d0+cos(pi/3.d0))/6.d0
      DL(2,icomp,1)=sin(pi/3.d0)/6.d0
...
*************************************************
 80   continue
*
* diamond (111) as fcc - ABBCCA stacking       !nstack.eq.8
*
      DL(1,icomp,1)=0.d0
      DL(2,icomp,1)=0.d0
...
* variable declarations:
      integer l,i,j,ilcs,ij1
      REAL*8 RMF(lcs),RFF(lcs),RMUF,omega,pi,rcore,rsnm
